...
(function(){
  ...
  var S={auth:null,group:'',rows:[],names:[],matches:{},confirmed:false};
  ...
  function coupons(start,mat){var out=[],cur=new Date(mat),guard=0;while(cur>start&&guard++<400){out.unshift(new Date(cur));cur=addMonths(cur,-6)}if(!out.length||out[0]>start)out.unshift(new Date(start));return out}
  function outstanding(loan,at){
    if(at<loan.start||at>=loan.maturity)return 0;
    if(loan.method==='MATURITY')return loan.principal;
    var cs=coupons(loan.start,loan.maturity),full=cs.length-1;if(full<1)return loan.principal;
    ...
    var r=loan.rate/200,pmt=(r*loan.principal)/(1-Math.pow(1+r,-full)),rem=loan.principal;
  ...
  function yours(){
    ...
    var byName={};inv.forEach(function(r){var k=S.matches[r.name]||r.name;byName[k]=(byName[k]||0)+r.amount/1e6});
    var exp=Object.keys(byName).map(function(k){return {id:k,v:byName[k]}}).sort(function(a,b){return b.v-a.v});
    ...
    exp.forEach(function(x){var e=D.cp.byId[x.id];if(e&&e.score!=null){scored.push({e:e,v:x.v});bands[e.band]=(bands[e.band]||0)+x.v}else unscored+=x.v});
  ...
  function borrowing(){
    ...
    var pasted=S.rows.filter(function(r){return r.side==='borrowing'&&r.start&&r.end}).map(function(r){return {method:/ANNUITY/.test(r.profile)?'ANNUITY':/EIP|EQUAL/.test(r.profile)?'EIP':'MATURITY',start:r.start,maturity:r.end,principal:r.amount,rate:r.rate||0,src:/pwlb/i.test(r.type)?'PWLB':'Market'}});
    ...
    return {loans:all,total:tot/1e6,pwlb:pwt/1e6,n:pw.length,rate:rate,life:life,mix:mix,proj:proj,refi:refi,refiSum:refiSum/1e6,refiNow:refiNow,refiWas:refiWas};
  }
  ...
  function render(){
    var Y=yours(),B=borrowing(),PA=peerAlloc(),PB=peerBorrow(),PI=peerIllustrative(),ps=peerSet();
    ...
    var html=
      ...
      panel('Credit risk on the Counterparty scale','ill','Weighted score of the banks and societies you hold, on the published method; funds, the DMADF and other authorities are not scored.',
        ...
        (credit?'<table class="plain"><thead><tr><th>Name</th><th class="num">Held</th><th class="num">Score</th><th>Agency rating</th></tr></thead><tbody>'+credit+'</tbody></table>':'<div class="empty">No name on the master list in this portfolio.</div>'))+
      ...
      panel('Outstanding balance, projected','real','Every loan run forward on its own repayment profile.',projection(B.proj),true)+
  ...
  function renderMatch(){
    var unsure=0,rows=S.names.map(function(nm){
      ...
      return '<tr'+(sure?'':' class="bad"')+'><td><b>'+esc(nm)+'</b></td><td class="num mono">'+fmtM(amt)+'</td><td class="match-cell"><select data-name="'+esc(nm)+'">'+opts+'</select></td><td>'+(sure?'<span class="chip chip-good">matched</span>':'<span class="chip chip-warn">check</span>')+'</td></tr>';
    }).join('');
  ...
  function confirm(){
    ...
    render();$('s4').scrollIntoView({behavior:'smooth',block:'start'});
  }
